...
var setNonInjectedPrestaToZero = function(openFiscaEntity, dateDeValeur) {
    ...
    var prestationsInjectees = _.pick(PRESTATIONS, function(format) {
        return format.type == Number;
    });

    _.forEach(prestationsInjectees, function(format, nomPresta) {
        if (! openFiscaEntity[nomPresta]) {
            openFiscaEntity[nomPresta] = {};
        }
        if(! openFiscaEntity[nomPresta][moisCourant]) {
            openFiscaEntity[nomPresta][moisCourant] = 0;
        }
    });
};
